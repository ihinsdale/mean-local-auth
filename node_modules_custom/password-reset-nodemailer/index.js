...
Forgot.prototype.middleware = function (req, res, next) {
    ...
    if (u.hostname !== this.mount.hostname ||
      ...
      u.pathname !== '/api/_password_reset') {
      // the point of this guard is to catch requests to the server's _password_reset route
      // the condition of u.pathname !== this.mount.pathname would work if our password
      // reset email linked directly there, but since our request to /api/_password_reset
      // is made by our Angular app, we have to make a custom condition
        next()
    }
    else if (!id) {
        res.statusCode = 400;
        ...
        //console.log('this.sessions, inside auth-token-expired block:', this.sessions);
        res.statusCode = 410;
