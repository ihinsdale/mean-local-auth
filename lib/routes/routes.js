...
module.exports = function(app) {
  ...
  // Auth-related routes except for password resetting
  ...
  app.route('/api/signup').post(function(req, res) {
    ...
    async.waterfall([
      ...
      if (error) {
        ...
        res.send(200, { username: req.user.username });
  ...
  app.get('/api/users/:username', function(req, res) {
    ...
    if (req.isAuthenticated() && req.user.username === requestedUsername) {
      res.send(200, { data: 'Public and private (i.e. authentication-required) info for user ' + requestedUsername + ' here' });
    } else {
      res.send(200, { data: 'Public info for user ' + requestedUsername + ' here' });
    }
  });
