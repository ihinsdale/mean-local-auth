...
module.exports = function(app) {
  ...
  app.get('/api/users/:username', function(req, res) {
    ...
    if (req.isAuthenticated() && req.user.username === requestedUsername) {
      res.send(200, 'Public and private (i.e. authentication-required) info for user ' + requestedUsername + ' here');
    } else {
      res.send(200, 'Public info for user ' + requestedUsername + ' here');
    }
  });
