...
var forgot = require('../../node_modules_custom/password-reset-nodemailer/index.js')({
  ...
  from : config.passwordResetSenderEmail,
  subject: config.publicDNS + ': Password reset request',
  transportType: 'SES',
  transportOptions: {
    AWSAccessKeyID: config.AWSSES.accessKeyId,
    AWSSecretKey: config.AWSSES.secretAccessKey,
    ServiceUrl: config.AWSSES.regionUrl
  }
});

module.exports = function(app) {
  app.use(forgot.middleware); // note that this step in effect creates
